...
function initBuffers2(gl) {
    ...
    const data = parseOBJ(obj_text);

    console.log(data);

    const vertCount = getVertCount(data.geometries[0].data.position);

    const positionBuffer = initPositionBuffer(gl, data.geometries[0].data.position);
    const textureCoordBuffer = initTextureBuffer(gl, data.geometries[0].data.texcoord);
    const normalBuffer = initNormalBuffer(gl, data.geometries[0].data.normal);
    //const colorBuffer = initColorBuffer(gl, data.geometries[0].data.color);
    //const indexBuffer = initIndexBuffer(gl, data.geometries.indices);
  ...
  function initPositionBuffer(gl, position) {
  ...
  */
  ...
  function initTextureBuffer(gl, texcoord) {
